...
async def handle_photo(message: Message, session: AsyncSession, state: FSMContext):
    """Обработчик получения фотографий от пользователей"""
    try:
        # Проверяем, не находится ли пользователь в процессе добавления доработки
        current_state = await state.get_state()
        if current_state and current_state.startswith("ImprovementStates"):
            # Если пользователь в процессе добавления доработки, пропускаем этот обработчик
            # Фото будет обработано в improvement_handlers.py
            return
        ...
        # Не отправляем ответное сообщение, чтобы не мешать пользователю
        pass
